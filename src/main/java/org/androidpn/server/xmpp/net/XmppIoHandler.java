...
import org.source.openfire.nio.XMLLightweightParser;
...
public class XmppIoHandler implements IoHandler {
    ...
    public void sessionCreated(IoSession session) throws Exception {
        ...
        //心跳包，验证服务器和客户端是否连接，设置session的idl
        session.getConfig().setUseReadOperation(true);
        //X秒后没有收到心跳包就断开session连接
//        int ListenDuration = 60;
//        session.getConfig().setIdleTime(IdleStatus.BOTH_IDLE, ListenDuration);
      
        
        Date date = new Date();
        SimpleDateFormat dateFormater = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        System.out.println(dateFormater.format(date) +" 创建一个链接session："+session.toString());
       
       // contentToTxt( "c:\\pnlog.txt",dateFormater.format(date) +" 创建一个链接session："+session.toString());
    }
    ...
    public void sessionIdle(IoSession session, IdleStatus status) throws Exception {
        ...
        Date date = new Date();
        SimpleDateFormat dateFormater = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        System.out.println(dateFormater.format(date)+" session 空闲...:"+session.toString());
    }
